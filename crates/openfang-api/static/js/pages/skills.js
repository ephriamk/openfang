// EphItUpStudio Skills Page — OpenClaw/ClawHub ecosystem + local skills + MCP servers
'use strict';
...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await EphAPI.get('/api/skills');
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await EphAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await EphAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await EphAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await EphAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        EphToast.error('Failed to load skill details');
      }
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await EphAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug) + '/code');
        this.skillCode = data.code || '';
        this.skillCodeFilename = data.filename || 'source';
        this.showSkillCode = true;
      } catch(e) {
        EphToast.error('Could not load skill source code');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await EphAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          EphToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          EphToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          EphToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          EphToast.error('Skill blocked by security scan');
        } else {
          EphToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      var self = this;
      EphToast.confirm('Uninstall Skill', 'Uninstall skill "' + name + '"? This cannot be undone.', async function() {
        try {
          await EphAPI.post('/api/skills/uninstall', { name: name });
          EphToast.success('Skill "' + name + '" uninstalled');
          await self.loadSkills();
        } catch(e) {
          EphToast.error('Failed to uninstall skill: ' + e.message);
        }
      });
    },
    ...
    async createDemoSkill(skill) {
      try {
        await EphAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        EphToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        EphToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await EphAPI.get('/api/mcp/servers');
