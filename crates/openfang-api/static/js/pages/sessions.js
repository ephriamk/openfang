...
function sessionsPage() {
  return {
    ...
    async loadSessions() {
      ...
      this.loadError = '';
      try {
        var data = await EphAPI.get('/api/sessions');
    ...
    deleteSession(sessionId) {
      var self = this;
      EphToast.confirm('Delete Session', 'This will permanently remove the session and its messages.', async function() {
        try {
          await EphAPI.del('/api/sessions/' + sessionId);
          self.sessions = self.sessions.filter(function(s) { return s.session_id !== sessionId; });
          EphToast.success('Session deleted');
        } catch(e) {
          EphToast.error('Failed to delete session: ' + e.message);
        }
      });
    },
    ...
    async loadKv() {
      ...
      this.memLoadError = '';
      try {
        var data = await EphAPI.get('/api/memory/agents/' + this.memAgentId + '/kv');
    ...
    async addKey() {
      ...
      try {
        await EphAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.newKey), { value: value });
        this.showAdd = false;
        EphToast.success('Key "' + this.newKey + '" saved');
        this.newKey = '';
        this.newValue = '""';
        await this.loadKv();
      } catch(e) {
    ...
    deleteKey(key) {
      var self = this;
      EphToast.confirm('Delete Key', 'Delete key "' + key + '"? This cannot be undone.', async function() {
        try {
          await EphAPI.del('/api/memory/agents/' + self.memAgentId + '/kv/' + encodeURIComponent(key));
          EphToast.success('Key "' + key + '" deleted');
          await self.loadKv();
        } catch(e) {
          EphToast.error('Failed to delete key: ' + e.message);
        }
      });
    },
    ...
    async saveEdit() {
      ...
      try {
        await EphAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.editingKey), { value: value });
        EphToast.success('Key "' + this.editingKey + '" updated');
        this.editingKey = null;
        this.editingValue = '';
        await this.loadKv();
      } catch(e) {
