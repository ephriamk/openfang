...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await EphAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await EphAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          EphToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await EphAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            EphToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await EphAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await EphAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            EphToast.success(this.setupModal.display_name + ' activated!');
          } else {
            EphToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          EphToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
    ...
    async removeChannel() {
      ...
      var self = this;
      EphToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await EphAPI.delete('/api/channels/' + name + '/configure');
          EphToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await EphAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          EphToast.success(result.message);
        } else {
          EphToast.error(result.message);
        }
      } catch(e) {
        EphToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        EphToast.success('Copied to clipboard');
      } catch(e) {
        EphToast.error('Copy failed');
      }
    },
