// EphItUpStudio Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await EphAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await EphAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        EphToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        EphToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await EphAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        EphToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        EphToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await EphAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        EphToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        EphToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      EphToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await EphAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          EphToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          EphToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await EphAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          EphToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          EphToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        EphToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await EphAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        EphToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        EphToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      EphToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await EphAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          EphToast.success('Trigger deleted');
        } catch(e) {
          EphToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
