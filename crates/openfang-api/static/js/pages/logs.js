// EphItUpStudio Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    async fetchLogs() {
      ...
      try {
        var data = await EphAPI.get('/api/audit/recent?n=200');
    ...
    async verifyChain() {
      try {
        var data = await EphAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          EphToast.success('Audit chain verified — ' + (data.entries || 0) + ' entries valid');
        } else {
          EphToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        EphToast.error('Chain verification failed: ' + e.message);
      }
    },
