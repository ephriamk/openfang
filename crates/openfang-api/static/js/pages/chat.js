  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            EphAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      EphAPI.get('/api/models').then(function(data) {
        ...
        EphToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      EphAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function(resp) {
        ...
        EphToast.success('Switched to ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        EphToast.error('Switch failed: ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      EphAPI.get('/api/commands').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            EphAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              EphToast.success('Session reset');
            }).catch(function(e) { EphToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            EphAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { EphToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            EphAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { EphToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && EphAPI.isWsConnected()) {
            EphAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
        ...
        case '/verbose':
          if (self.currentAgent && EphAPI.isWsConnected()) {
            EphAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected. Connect to an agent first.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && EphAPI.isWsConnected()) {
            EphAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
          ...
          break;
        case '/status':
          EphAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              EphAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function(resp) {
                ...
                self.scrollToBottom();
              }).catch(function(e) { EphToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          EphAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          EphAPI.get('/api/network/status').then(function(ns) {
          ...
          break;
        case '/a2a':
          EphAPI.get('/api/a2a/agents').then(function(res) {
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await EphAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await EphAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await EphAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof EphToast !== 'undefined') EphToast.success('New session created');
      } catch(e) {
        if (typeof EphToast !== 'undefined') EphToast.error('Failed to create session');
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await EphAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof EphToast !== 'undefined') EphToast.error('Failed to switch session');
      }
    },
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await EphAPI.upload(this.currentAgent.id, att.file);
            ...
            EphToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      // HTTP fallback
      if (!EphAPI.isWsConnected()) {
        EphToast.info('Using HTTP mode (no streaming)');
      }
      ...
      try {
        ...
        var res = await EphAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      EphAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      EphToast.confirm('Stop Agent', 'Stop agent "' + name + '"? The agent will be shut down.', async function() {
        try {
          await EphAPI.del('/api/agents/' + self.currentAgent.id);
          EphAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          EphToast.success('Agent "' + name + '" stopped');
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          EphToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          EphToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof EphToast !== 'undefined') EphToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await EphAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof EphToast !== 'undefined') EphToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
