// EphItUpStudio Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
...
function settingsPage() {
  return {
    ...
    async loadSysInfo() {
      try {
        var ver = await EphAPI.get('/api/version');
        var status = await EphAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await EphAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await EphAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await EphAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await EphAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await EphAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await EphAPI.post('/api/models/custom', {
          id: id,
    ...
    async deleteCustomModel(modelId) {
      ...
      try {
        await EphAPI.del('/api/models/custom/' + encodeURIComponent(modelId));
        EphToast.success('Model deleted');
        await this.loadModels();
      } catch(e) {
        EphToast.error('Failed to delete: ' + (e.message || 'Unknown error'));
      }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          EphAPI.get('/api/config/schema').catch(function() { return {}; }),
          EphAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await EphAPI.post('/api/config/set', { path: path, value: value });
        this.configDirty[key] = false;
        EphToast.success('Saved ' + field);
      } catch(e) {
        EphToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { EphToast.error('Please enter an API key'); return; }
      try {
        await EphAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        EphToast.success('API key saved for ' + provider.display_name);
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        EphToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await EphAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        EphToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        EphToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await EphAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        EphToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await EphAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            EphToast.success('GitHub Copilot authenticated successfully!');
            ...
            EphToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            EphToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            EphToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          EphToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await EphAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          EphToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          EphToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        EphToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { EphToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        EphToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await EphAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          EphToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          EphToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        EphToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async addCustomProvider() {
      ...
      if (!name) { EphToast.error('Please enter a provider name'); return; }
      var url = this.customProviderUrl.trim();
      if (!url) { EphToast.error('Please enter a base URL'); return; }
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        EphToast.error('URL must start with http:// or https://'); return;
      }
      ...
      try {
        var result = await EphAPI.put('/api/providers/' + encodeURIComponent(name) + '/url', { base_url: url });
        if (this.customProviderKey.trim()) {
          await EphAPI.post('/api/providers/' + encodeURIComponent(name) + '/key', { key: this.customProviderKey.trim() });
        }
        this.customProviderName = '';
        this.customProviderUrl = '';
        this.customProviderKey = '';
        this.customProviderStatus = '';
        EphToast.success('Provider "' + name + '" added' + (result.reachable ? ' (reachable)' : ' (not reachable yet)'));
        await this.loadProviders();
      } catch(e) {
        this.customProviderStatus = 'Error: ' + (e.message || 'Failed');
        EphToast.error('Failed to add provider: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await EphAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await EphAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await EphAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await EphAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await EphAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          EphToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        EphToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await EphAPI.post('/api/migrate', {
          source: 'openclaw',
