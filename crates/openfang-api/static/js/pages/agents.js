...
function agentsPage() {
  return {
    ...
    async loadSpawnProfiles() {
      ...
      try {
        var data = await EphAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          EphAPI.get('/api/templates'),
          EphAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    async showDetail(agent) {
      ...
      try {
        var full = await EphAPI.get('/api/agents/' + agent.id);
    ...
    killAgent(agent) {
      var self = this;
      EphToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await EphAPI.del('/api/agents/' + agent.id);
          EphToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          EphToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },
    ...
    killAllAgents() {
      ...
      EphToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await EphAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          EphToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          EphToast.success(list.length + ' agent(s) stopped');
        }
      });
    },
    ...
    nextStep() {
      if (this.spawnStep === 1 && !this.spawnForm.name.trim()) {
        EphToast.warn('Please enter an agent name');
        return;
      }
    ...
    async setMode(agent, mode) {
      try {
        await EphAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        EphToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        EphToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        EphToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await EphAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            EphAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            EphAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          EphToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          EphToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        EphToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await EphAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        EphToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await EphAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        EphToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await EphAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        EphToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        EphToast.error('Failed to save file: ' + e.message);
      }
    ...
    async saveConfig() {
      ...
      try {
        await EphAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        EphToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        EphToast.error('Failed to save config: ' + e.message);
      }
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await EphAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          EphToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        EphToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(name) {
      try {
        var data = await EphAPI.get('/api/templates/' + encodeURIComponent(name));
        if (data.manifest_toml) {
          var res = await EphAPI.post('/api/agents', { manifest_toml: data.manifest_toml });
          if (res.agent_id) {
            EphToast.success('Agent "' + (res.name || name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        EphToast.error('Failed to spawn from template: ' + e.message);
      }
    },
    ...
    async clearHistory(agent) {
      var self = this;
      EphToast.confirm('Clear History', 'Clear all conversation history for "' + agent.name + '"? This cannot be undone.', async function() {
        try {
          await EphAPI.del('/api/agents/' + agent.id + '/history');
          EphToast.success('History cleared for "' + agent.name + '"');
        } catch(e) {
          EphToast.error('Failed to clear history: ' + e.message);
        }
      });
    },
    ...
    async changeModel() {
      ...
      try {
        var resp = await EphAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: this.newModelValue.trim() });
        var providerInfo = (resp && resp.provider) ? ' (provider: ' + resp.provider + ')' : '';
        EphToast.success('Model changed' + providerInfo + ' (memory reset)');
        ...
        EphToast.error('Failed to change model: ' + e.message);
      }
    ...
    async addFallback() {
      ...
      try {
        await EphAPI.patch('/api/agents/' + this.detailAgent.id + '/config', {
          fallback_models: this.detailAgent._fallbacks
        });
        EphToast.success('Fallback added: ' + provider + '/' + model);
      } catch(e) {
        EphToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async removeFallback(idx) {
      ...
      try {
        ...
        EphToast.success('Fallback removed');
      } catch(e) {
        EphToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async loadToolFilters() {
      ...
      try {
        this.toolFilters = await EphAPI.get('/api/agents/' + this.detailAgent.id + '/tools');
      } catch(e) {
    ...
    async saveToolFilters() {
      if (!this.detailAgent) return;
      try {
        await EphAPI.put('/api/agents/' + this.detailAgent.id + '/tools', this.toolFilters);
      } catch(e) {
        EphToast.error('Failed to update tool filters: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await EphAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          EphToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
